...
export default function HomePage() {
  ...
  return (
    <div className="min-h-[calc(100vh-56px)] flex flex-col relative overflow-hidden">
      ...
      <div className="absolute top-1/4 -left-32 w-[500px] h-[500px] rounded-full bg-aurora/10 blur-[120px] pointer-events-none" />
      <div className="absolute bottom-1/4 -right-32 w-[400px] h-[400px] rounded-full bg-plasma/10 blur-[100px] pointer-events-none" />

      {/* Hero Section */}
      <div className="flex-1 flex flex-col items-center justify-center text-center px-6 py-16">
        {/* Pill badge */}
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-aurora/30 bg-aurora/5 text-[11px] text-aurora-bright font-mono tracking-widest uppercase mb-10">
          <span className="text-habitable">●</span> Deep Space Transit Classifier
        </div>

        {/* Title */}
        <h1 className="font-display text-6xl md:text-8xl font-bold tracking-tight leading-[1.05] mb-8">
          Explore new worlds
          <br />
        ...
        {/* Subtitle */}
        <p className="text-text-secondary text-lg md:text-xl max-w-2xl mx-auto leading-relaxed font-sans mb-16">
          Exovision maps candidate light curves from the NASA Kepler catalog and reconstructs target systems in physically derived 3D orbital environments.
        </p>
        ...
        <div className="w-full max-w-2xl mx-auto relative">
          <form onSubmit={handleSubmit} className="w-full">
            <div className="relative group">
              ...
              <div className="glass relative flex items-center rounded-2xl p-1.5 transition-all focus-within:border-aurora/40 shadow-2xl">
          ...
          {dropdownOpen && (
            <div className="absolute top-full left-0 right-0 mt-3 glass rounded-xl shadow-2xl max-h-80 overflow-y-auto z-50 py-2">
              {results.map((e) => (
                <button
                  key={e.koi_name}
                  onClick={() => {
      ...
      {/* Footer strip */}
      <div className="border-t border-border px-6 py-4 flex items-center justify-between text-xs text-text-muted">
        <div className="flex items-center gap-2">
